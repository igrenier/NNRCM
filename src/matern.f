...
c         , ONLY: C_DOUBLE, C_INT
        
c        integer(C_INT), value, INTENT(IN) :: n
c        real(C_DOUBLE), POINTER, intent(in) :: theta(:) ,x(:)
c        real(C_DOUBLE), allocatable, intent(out) :: y(:)
...
c
        ...
        parameter( zero = 0.0d0 , fifty = 50.0d0 )
c
        dimension theta(2),x(n),y(n)
        dimension bk(50)
	integer it2,i,ncalc
c
        do i = 1, n
          if( x(i) .le. zero ) then
            ...
            if(theta(2) .lt. fifty) then
              cnv = dexp((theta(2)-1.d0)*dlog(2.d0))*dgamma(theta(2))
              it2 = dint(theta(2))
	      arg = x(i)/theta(1)
              call rkbesl(arg,theta(2)-it2,it2+1,1,bk,ncalc)
              y(i) = dexp(theta(2)*dlog(arg))*bk(it2+1)/cnv
            else
	      write(*,*) 'ERROR (rkmat):'
	      write(*,*) 'Smoothness parameter too high'
	      write(*,*) 'Use Gaussian correlation function'
	      stop
            endif
          endif
        enddo
c
        ...
        subroutine derivmat(theta,x,y,n)
c	--------------------------------

c	Subroutine that calculates the derivative of the
c	Matern correlation function
c	7/8/99

        implicit double precision (a-h,o-z)
        parameter( zero = 0.0d0 , fifty = 50.0d0 , one = 1.d0)
c
        dimension theta(2),x(n),y(n)
        dimension bk(50)
	integer it2,i,ncalc
c
        do i = 1, n
          if( x(i) .le. zero ) then
            ...
            if(theta(2) .lt. fifty) then
	      if(theta(2) .lt. one) then
	        theta2 = one - theta(2)
	      else
	        theta2 = theta(2) - one
	      endif
              cnv = dexp((theta(2)-one)*dlog(2.d0))*dgamma(theta(2))
              it2 = dint(theta2)
	      arg = x(i)/theta(1)
              call rkbesl(arg,theta2-it2,it2+1,1,bk,ncalc)
              y(i) = dexp((theta(2)+one)*dlog(arg))*bk(it2+1)
	      y(i) = y(i)/(cnv*theta(1))
            else
	      write(*,*) 'ERROR (rkmat):'
	      write(*,*) 'Smoothness parameter too high'
